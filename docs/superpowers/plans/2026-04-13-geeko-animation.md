# Geeko Animation Replacement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace static dashboard Geeko with animated inline SVG using CSS animations for tail sway, head bob, blinking, and pupil movement.

**Architecture:** Inline SVG in DashboardLeftPanel component with global CSS animations targeting specific SVG element IDs. Animations use `transform-box: fill-box` for proper SVG coordinate space transformations.

**Tech Stack:** React, TypeScript, CSS animations, inline SVG
...
- [ ] **Step 1: Add Geeko animation section to globals.css**

Open `src/styles/globals.css` and append the following at the end of the file (after line 280):
...
#gp-tail { 
  animation: geekoSway 6s infinite; 
}

#gp-pupil { 
  animation: geekoLook 4s infinite; 
}
...
#gp-eyelids { 
  animation: geekoBlink 2s infinite; 
}

#gl-head { 
  animation: geekoSway 5s infinite;
  transform-origin: bottom left; 
}

#gl-eyelids { 
  animation: geekoBlink 3s infinite; 
}
...
- [ ] **Step 3: Commit CSS animations**

```bash
git add src/styles/globals.css
git commit -m "feat: add Geeko SVG animation keyframes and selectors"
```

---

## Task 2: Replace Image with Inline SVG

**Files:**
- Modify: `src/components/DashboardLeftPanel.tsx:119`
- Reference: `public/geeko/geeko-sitting-up.svg` (source)

- [ ] **Step 1: Replace img tag with inline SVG**

In `src/components/DashboardLeftPanel.tsx`, replace line 119:
...
- [ ] **Step 3: Start dev server and check visual rendering**

Run: `npm run dev`
Navigate to: `http://localhost:5173` (or your dev server URL)
Expected: Dashboard displays Geeko with animations (tail swaying, head bobbing, eyes blinking/looking)

- [ ] **Step 4: Commit inline SVG replacement**

```bash
git add src/components/DashboardLeftPanel.tsx
git commit -m "feat: replace static Geeko image with animated inline SVG"
```

---

## Task 3: Remove Old SVG File

**Files:**
- Delete: `public/dashboard-geeko.svg`

- [ ] **Step 1: Delete old dashboard-geeko.svg**

```bash
rm public/dashboard-geeko.svg
```

- [ ] **Step 2: Verify file is deleted**

Run: `ls public/dashboard-geeko.svg`
Expected: "No such file or directory"

- [ ] **Step 3: Check dist folder if it exists**

```bash
rm -f dist/dashboard-geeko.svg
```
...
```bash
git add -A
git commit -m "chore: remove unused dashboard-geeko.svg"
```
...
**Files:**
- Test: Dashboard page (main view)
- Test: Conference display mode
...
Verify:
- Geeko tail sways smoothly (6s cycle, 10deg rotation)
- Geeko head bobs (6s cycle, bottom-left origin)
- Eyelids blink (2s cycle, stroke-width 0→120→0)
- Pupils move (4s cycle, translating around eye)
- No layout shifts or overflow
- SVG scales with maxWidth constraint

- [ ] **Step 2: Test conference display mode**

Navigate to: Conference display URL (if different)
Verify:
- Same animations work in left panel
- Animations continue during page transitions
...
Expected: No errors or warnings related to SVG or CSS animations

---

## Success Criteria Checklist

- [ ] Dashboard displays animated Geeko with tail sway, head bob, blinking, and eye movement
- [ ] Conference display mode shows same animations
- [ ] No layout shifts or visual regressions
- [ ] Animations run smoothly at 60fps
- [ ] Old `dashboard-geeko.svg` file removed from public folder
